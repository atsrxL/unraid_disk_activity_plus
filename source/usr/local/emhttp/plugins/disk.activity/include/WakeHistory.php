...
if ($fp) {
    while (($line = fgets($fp)) !== false) {
        ...
        // Only HIGH events are credited to a named source. MEDIUM remains visible
        // in history but is grouped as unknown for source/share statistics.
...
usort($sourceList, function ($a, $b) {
    $countCmp = ($b['count7d'] ?? 0) <=> ($a['count7d'] ?? 0);
